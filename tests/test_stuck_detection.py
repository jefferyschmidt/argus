...
def _worker(reply_text="NONE"):
    ...
    speak_fn = MagicMock()
    lock = threading.Lock()
    return StuckDetectionWorker(router, speak_fn, lock), router, speak_fn
...
def test_window_switch_resets_and_does_not_assess():
    worker, router, speak_fn = _worker()
...
def test_long_duration_with_none_reply_produces_no_delivery():
    worker, router, speak_fn = _worker(reply_text="NONE")
    worker._current_title = "VS Code"
...
def test_long_duration_with_real_reply_is_delivered():
    worker, router, speak_fn = _worker(reply_text="Looks like that build's been failing a few times in a row.")
    worker._current_title = "VS Code"
    _past_threshold(worker)
    ...
    assert "failing" in speak_fn.call_args[0][0]
...
def test_deliver_skips_when_interaction_lock_is_held():
    ...
    lock = threading.Lock()
    lock.acquire()
    worker = StuckDetectionWorker(router, speak_fn, lock)
